...
def sample_trajectory(env, policy, max_path_length, render=False):
    ...
    while True:
        ...
        ac_tensor = policy(ob_tensor).sample()  # HINT: this is a numpy array????wrong!
        ac = ptu.to_numpy(ac_tensor)

        # TODO: take that action and get reward and next ob
        next_ob, rew, done, _ = env.step(ac)  # HINT: this is a tuple of (next_ob, rew, done, info)
        ...
        if steps == max_path_length or done:
            ...
            rollout_done = 0
        # print(type(next_ob), type(ac), type(ob))
