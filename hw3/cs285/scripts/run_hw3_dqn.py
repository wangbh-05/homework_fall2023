    ...
    for step in tqdm.trange(config["total_steps"], dynamic_ncols=True):
        ...
        if isinstance(replay_buffer, MemoryEfficientReplayBuffer):
            ...
            replay_buffer.insert(
                ...
                # according to the Hint in pdf,when truncated,
                # we should reset the env but the flag for TD-updates(stored in replay buffer)
                # should be false
                done=done and not truncated # in TD learning, we only use the true done flag, not including truncated
            )
        else:
